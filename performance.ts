...
const canvass = <HTMLCanvasElement>document.getElementById("canvas");
...
class Sortt {
    ...
    static insertionSort(values) {
        for (var i = 1; i < values.length; i++) {
            if (values[i] < values[0]) values.unshift(values.splice(i, 1)[0]);
            else if (values[i] > values[i - 1]) continue;
            else {
                for (var j = 1; j < i; j++) {
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    for (let i = 0; i < lengthes.length; i++) {
        ...
        if (window.location.hash == "#bubble") Sortt.bubbleSort(values);
        if (window.location.hash == "#selection")
            Sortt.selectionSort(values, function (a, b) { return a - b; });
        if (window.location.hash == "#insertion") Sortt.insertionSort(values);
        if (window.location.hash == "#heap") Sortt.heapSort(values);
        if (window.location.hash == "#quick") Sortt.quicksort(values);
        if (window.location.hash == "#merge") Sortt.mergeSort(values);
